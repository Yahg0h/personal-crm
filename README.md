...
```
After running this command in the terminal, a secure secret key will appear below the command. Copy this key and paste it in place of "your_secret_key_here" in the `.env` file.
> **Note for Windows users:** Do not use the `echo` command to create the `.env` file, as PowerShell saves it in UTF-16 encoding which causes errors. Instead, create the file manually using a text editor (Notepad, VS Code, etc.) and make sure to save it as **UTF-8**.

> **It is required to create a .env file with a secret key created or generated by you for the application to function;** otherwise, you will receive `RuntimeError: A secret key is required to use CSRF.` in your terminal and an error page on the application.
